...
while True:
    ...
    for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
        ...
        if matches[best_match_index]:
            ...
            # Red rectangle and 'Unknown' label for unidentified faces
            cv2.rectangle(frame, (left, top), (right, bottom), (0, 0, 255), 2)
            text_size, _ = cv2.getTextSize("Unknown", cv2.FONT_HERSHEY_SIMPLEX, 0.8, 2)
            text_x = left + (right - left - text_size[0]) // 2
            cv2.putText(frame, "Unknown", (text_x, top - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
